...
A_End_Morphology <- function (input, output, session){
  ...
  if(ncol(Nodes %>% select(starts_with("EndType"))) >= 1){
    ...
    Plus_end_morphology_Pole1 <<- End_distribution_Plus(which(colnames(Segments) == "Pole1_00"), 
                                                        1)
    Minus_end_morphology_Pole1 <<- End_distribution_Minus(which(colnames(Segments) == "Pole1_00"), 
                                                          1)
    ...
    tryCatch({
      Plus_end_morphology_Pole2 <<- End_distribution_Plus(which(colnames(Segments) == "Pole2_00"),
                                                          2)
      Minus_end_morphology_Pole2 <<- End_distribution_Minus(which(colnames(Segments) == "Pole2_00"),
                                                            2)
    },
    error = function(e){}
    )
