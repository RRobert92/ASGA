...
IKD <- ggplot(Data_1_IKD, aes("Metaphase #1", `Inter-kinetochore distance`)) + 
       ...
       theme_classic()
IKD <- IKD + 
       geom_quasirandom(data = Data_2_IKD, aes("Metaphase #2", `Inter-kinetochore distance`), color = "brown3") + 
       stat_summary(data = Data_2_IKD, 
                    aes("Metaphase #2", `Inter-kinetochore distance`), 
                    fun.data="mean_sdl", fun.args = list(mult=1), geom="pointrange")
IKD <- IKD + 
       geom_quasirandom(data = Data_3_IKD, aes("Metaphase #3", `Inter-kinetochore distance`), color = "brown4") +
...
IKD_KMT_NO <- IKD_KMT_NO +
              ...
              geom_smooth(data = Data_2_IKD_KMT_No, aes(`KMTs no.`, `Inter-kinetochore distance`), method = "lm", color = "brown3", se = F)
IKD_KMT_NO <- IKD_KMT_NO +
...
Tortuosity <- Tortuosity + 
              geom_quasirandom(data = Data_2_KMT_Total_Curv, aes("Metaphase #2", Curvature), method = "tukeyDense", color = "brown3") + 
              stat_summary(data = Data_2_KMT_Total_Curv,
                           aes("Metaphase #2", Curvature), 
                           fun.data="mean_sdl", fun.args = list(mult=1), geom="pointrange")
Tortuosity <- Tortuosity + 
              geom_quasirandom(data = Data_3_KMT_Total_Curv, aes("Metaphase #3", Curvature), method = "tukeyDense", color = "brown4") + 
              stat_summary(data = Data_3_KMT_Total_Curv,
                           aes("Metaphase #3", Curvature), 
                           fun.data="mean_sdl", fun.args = list(mult=1), geom="pointrange")
Tortuosity <- Tortuosity + 
              geom_quasirandom(data = Tortuosity_avg, aes("Avg.", Curvature), method = "tukeyDense", color = "grey20") + 
