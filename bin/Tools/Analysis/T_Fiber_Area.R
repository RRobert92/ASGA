...
polygon_area <- function(x){
  ...
  i=1
  for (i in 1:as.numeric(nrow(get(paste(colnames(Segments)[x], "fiber", sep = "_"))))) {
    MT_no[i,1] <- ncol(select(get(paste(colnames(Segments)[x], "fiber",  sep = "_")), 
                              starts_with("V"))) - sum(is.na(select(get(paste(colnames(Segments)[x], "fiber",  sep = "_")), 
                                                                    starts_with("V"))[i,]))
    DF <- data.frame(X_Coord = as.numeric(),
                     Y_Coord = as.numeric(),
                     Z_Coord = as.numeric())
...
Mean_DF <- get(paste(colnames(Segments)[x], "fiber", sep = "_"))[3:5]
DF_full <- data.frame()
for(i in 1:nrow(DF)){
  dist <- data.frame()
  for(j in 1:ncol(DF)){
  ...
  dist$distance <- apply(dist, 1, function(y)dist(matrix(y, nrow = 2, byrow = TRUE)))
