...
Points <- read_excel("C:/Users/kerth/Desktop/Metaphase_1.am.xlsx", sheet = "Points")
Points <- data.frame(
  Point_ID = c(Points$`Point ID`),
  X_Coord = c(Points$`X Coord`) / 10000,
  Y_Coord = c(Points$`Y Coord`) / 10000,
  Z_Coord = c(Points$`Z Coord`) / 10000)

Segments <- read_excel("C:/Users/kerth/Desktop/Metaphase_1.am.xlsx", sheet = "Segments")
...
leading_KMTs <- function(x) {
  ...
  while (j <= as.numeric(nrow(get(colnames(Segments)[x])))) {
    leading[j, ] <- as.numeric(nrow(get(paste(
    colnames(Segments)[x], j, sep = "_"))))
    j = j + 1
  }
...
leading_KMTsv2 <- function(x, y) {
  ...
  while (j <= as.numeric(nrow(get(colnames(Segments)[x])))) {
    KMT_lenght <-
      Segments[as.numeric(get(colnames(Segments)[x])[j, 1] + 1), as.numeric(ncol(Segments) - 3)] /
      10000
...
find_polygon <- function(x) {
  ...
  while (i <= as.numeric(nrow(get(paste(
    colnames(Segments)[x], "fiber", sep = "_"))))) {
...
for (i in 2:as.numeric(ncol(Segments) - 4)) {
  ...
  j = 1
  while (j <= as.numeric(nrow(get(colnames(Segments)[i])))) {
    assign(paste(colnames(Segments)[i], j, sep = "_"), Find_XYZ(get(paste(
      colnames(Segments)[i], j, sep = "_"
    ))))
    j = j + 1
  }
  ...
  while (j <= as.numeric(nrow(get(colnames(Segments)[i])))) {
    assign(paste(colnames(Segments)[i], j, sep = "_"),
           Sort_by_distance_to_pole(get(paste(
             colnames(Segments)[i], j, sep = "_"
           ))))
    j = j + 1
  }
