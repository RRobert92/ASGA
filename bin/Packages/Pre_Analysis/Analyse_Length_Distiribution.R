...
LD_P1 <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole1_00")]))["length"]
Plus_end <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole1_00")]))["plus_dist_to_kinetochore_core"]
Dist_pole <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole1_00")]))["plus_dist_to_pole"]
Elips <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole1_00")]))["Elipse_Position"]
Minus_dist <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole1_00")]))["minus_dist_to_pole"]
LD_P1 <- cbind(LD_P1,
               Plus_end,
               Dist_pole,
               Elips,
...
for (i in as.numeric(which(colnames(Segments_KMT) == "Pole1_00")+1) : as.numeric(which(colnames(Segments_KMT) == "Pole2_00") - 1)){
  tryCatch({
  DF_LD <- get(paste(colnames(Segments_KMT)[i]))["length"]
  DF_Plus_end <- get(paste(colnames(Segments_KMT)[i]))["plus_dist_to_kinetochore_core"]
  DF_Dist_pole <- get(paste(colnames(Segments_KMT)[i]))["plus_dist_to_pole"]
  DF_Elips <- get(paste(colnames(Segments_KMT)[i]))["Elipse_Position"]
  Minus_dist <- get(paste(colnames(Segments_KMT)[i]))["minus_dist_to_pole"]
  DF <- cbind(DF_LD,
              DF_Plus_end,
              DF_Dist_pole,
              DF_Elips,
...
####################
  ...
  tryCatch({
    LD_P2 <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole2_00")]))["length"]
    Minus_dist <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole2_00")]))["minus_dist_to_pole"]
    Plus_end <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole2_00")]))["plus_dist_to_kinetochore_core"]
    Dist_pole <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole2_00")]))["plus_dist_to_pole"]
    Elips <- get(paste(colnames(Segments_KMT)[which(colnames(Segments_KMT) == "Pole2_00")]))["Elipse_Position"]
    LD_P2 <- cbind(LD_P2,
                   Plus_end,
                   Dist_pole,
                   Elips,
